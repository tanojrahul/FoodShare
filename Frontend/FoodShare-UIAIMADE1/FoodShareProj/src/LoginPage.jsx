...
const RoleSelector = ({ selectedRole, onChange }) => {
  ...
  return (
    <div className="mb-6">
      <label className="block text-white text-sm mb-2">Select Your Role</label>
      <div className="grid grid-cols-2 gap-3">
        {roles.map((role) => (
          <div
            key={role.id}
            onClick={() => onChange(role.id)}
            className={`cursor-pointer p-3 rounded-lg flex items-center justify-center ${
              selectedRole === role.id
                ? 'bg-[#D4C9BE] text-[#123458] font-medium'
