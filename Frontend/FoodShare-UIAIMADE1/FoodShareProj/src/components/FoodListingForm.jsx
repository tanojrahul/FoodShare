...
import { FiX, FiAlertCircle, FiCamera, FiUpload, FiCalendar } from 'react-icons/fi';

// Import services
import { createFoodListing } from '../services/donationService';
import { sendInAppNotification, sendSystemNotification } from '../services/notificationService';
...
const FoodListingForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use the donationService method for creating a food listing
      const data = {
        ...
        description: formData.description || '' // Make description optional
      };
      
      const createdDonation = await createFoodListing(data);
      
      // Send notification to administrators about new donation
      await sendInAppNotification(
        'admin', // Generic admin notification
        'New Donation Listed',
        `A new donation "${formData.title}" has been listed by ${currentUser?.name || 'a donor'}.`,
        'info',
        { donationId: createdDonation.id, linkTo: `/admin/donations/${createdDonation.id}` }
      );
      
      // Send system notification to nearby beneficiaries
      // This will notify beneficiaries in the area about this new donation
      await sendSystemNotification(
        `New Food Available: ${formData.title}`,
        `${formData.quantity} units of ${formData.category} are now available near ${formData.pickupLocation}.`,
        'info',
        ['beneficiary', 'ngo']
      );
