...
const FoodCard = ({ food, isLoading, onClick }) => {
  ...
  return (
    ...
    >
      {/* Food Image */}
      {food.imageUrl ? (
        <div className="h-32 sm:h-40 w-full overflow-hidden rounded-t-lg">
          <img 
            src={food.imageUrl} 
            alt={food.type} 
        ...
        <div className="h-32 sm:h-40 w-full bg-[#D4C9BE] bg-opacity-30 rounded-t-lg flex items-center justify-center">
          <FiPackage size={30} className="sm:text-3xl md:text-4xl text-[#123458] opacity-40" />
        </div>
      )}
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-semibold text-base sm:text-lg text-[#123458] line-clamp-1">{food.title}</h3>
          <StatusBadge status={food.status} />
        </div>

        <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-gray-600 mb-auto">
          <div className="flex items-center">
            <FiPackage className="mr-1 sm:mr-2 text-[#123458]" />
            <span>{food.quantity} {food.unit || 'items'}</span>
          </div>
          
          <div className="flex items-center">
            <FiCalendar className="mr-1 sm:mr-2 text-[#123458]" />
            <span>Expires: <span className={getUrgencyClass()}>{formatExpirationDate()}</span></span>
          </div>
          
          {food.location && (
            <div className="flex items-center">
              <FiMapPin className="mr-1 sm:mr-2 text-[#123458]" />
        ...
        <Link 
          ...
          onClick={onClick}
          className="mt-3 sm:mt-4 flex items-center justify-center px-3 sm:px-4 py-1.5 sm:py-2 bg-[#123458] text-white text-sm sm:text-base rounded hover:bg-opacity-90 transition-colors"
        >
